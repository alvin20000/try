/*
  ...
  3. Functions
    - `register_user` - Register new user with email, password, and full name
    - `authenticate_user` - Authenticate user login
    - `update_user_profile` - Update user profile information
*/

-- Create app_users table with simplified fields
...
-- Simplified user registration function
...
-- Function to create order with user context (simplified)
...
BEGIN
  ...
  -- Insert order with user information and provided contact details
  INSERT INTO orders (
    order_number,
    user_id,
    customer_name,
